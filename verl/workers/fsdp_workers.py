...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        from reward.model.modeling_qwen_2_5_par_reward import RewardModelQwenForCausalLM, MultiHeadMLP, RMSNorm
        ...
        with init_context(), warnings.catch_warnings():
            ...
            reward_module = RewardModelQwenForCausalLM.from_pretrained(
                pretrained_model_name_or_path=local_path,
            ...
            if config.model.get("use_remove_padding", False):
                warnings.warn(
                    "We recommend disabling `use_remove_padding` for VLLM qwen reward.",
        ...
        # avoiding embedding sharding error
        auto_wrap_policy = ModuleWrapPolicy(module_classes=list(layer_classes_to_wrap))
    ...
    def _adjust_by_gt(
        ...
        if not self.config.get("enable_gt_adjust", False) or not rewards:
            return rewards
        ...
        for i, (gt, ai) in enumerate(zip(gt_answers, ai_answers)):
            if gt is None or ai is None:
                continue
            if gt == ai:
                adjusted[i] += self.config.get("gt_match_bonus_ratio", 0.6) * span
            else:
                adjusted[i] -= self.config.get("gt_mismatch_penalty_ratio", 0.6) * span
        return adjusted
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        _eval_ai = EvalAIAnswerProcessor()
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            img = None
            if "<image>" in q:
                image_token = getattr(target_processor, "image_token", "<image>")
                q = image_token.join(q.split("<image>"))
                img = data.non_tensor_batch["original_image"][i]['image'][0]
            ...
            gt_p = _eval_ai(gt) if gt is not None else None
            ai_p = _eval_ai(response) if response is not None else None
            ...
            batch_conv.append(prompt_with_chat_template)
        
        max_length = self.config.get("max_length", src_max_length)
        if max_length is None:
            max_length = src_max_length

        model_ready = target_processor(
            text=batch_conv,
            images=batch_imgs,  # None or List[Image]
            return_tensors="pt",
            padding="max_length",
            truncation=True,
        ...
        return {k: v.to(get_device_id()) for k, v in model_ready.items() if torch.is_tensor(v)}, gt_parsed, ai_parsed
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if self.config.use_dynamic_bsz :
            raise NotImplementedError("Sorry, we donot support dynamic_bsz for VLLM + ValueHead architecture")
        

        # with self.ulysses_sharding_manager:
        for start in range(0, bsz, self.config.micro_batch_size_per_gpu):
            end = min(start + self.config.micro_batch_size_per_gpu, bsz)
            mb = data[start:end]

            if self._do_switch_chat_template:
                rm_data, gt_parsed, ai_parsed = self._switch_chat_template(mb)
            else:
                raise NotImplementedError("Sorry, we only support template switching for VLLM + ValueHead architecture")
            
            with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
                _multi, reward_logits = self.reward_module.predict(**rm_data)

            if reward_logits.dim() == 2 and reward_logits.size(-1) == 1:
                reward_logits = reward_logits.squeeze(-1)
        
            raw_reward = reward_logits.detach().float().cpu().tolist()
            output.extend(self._adjust_by_gt(raw_reward, gt_parsed,ai_parsed))
        
        scores = torch.tensor(output).to(get_device_id())
